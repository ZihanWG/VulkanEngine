...
- `bc7enc/` is Rich Geldreich's [bc7enc_rdo](https://github.com/richgel999/bc7enc_rdo), pinned at
  ...
  BC1-BC5 encoders (`rgbcx`) used by the offline texture cook, plus the BC7 decoder (`bc7decomp`) that
  `vecook --verify` decodes cooked files with. Only the files the cook needs are
