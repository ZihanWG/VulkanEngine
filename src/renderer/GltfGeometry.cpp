...
// The tinygltf implementation lives in GltfSkinnedImport.cpp, which is also part
// of VulkanEngineCore; here we only need the declarations.
...
GltfGeometry loadGltfGeometry(const std::filesystem::path& path, JobSystem* jobSystem)
{
    ...
    const auto createMeshFromSource = [&](size_t meshIndex) {
        ...
        CpuMeshData mesh;
        mesh.debugName = meshDebugName(sourceMesh, meshIndex);
        ...
        for (size_t primitiveIndex = 0; primitiveIndex < subMeshes.size(); ++primitiveIndex) {
            ...
            primitive.lodBase = static_cast<uint32_t>(mesh.lods.size());
            primitive.lodCount = static_cast<uint32_t>(primitiveLods.size());
            mesh.lods.insert(mesh.lods.end(), primitiveLods.begin(), primitiveLods.end());
        ...
        mesh.primitives = std::move(subMeshes);
    ...
    geometry.meshes = std::move(meshes);
    geometry.nodeMeshInstances = std::move(nodeMeshInstances);
    geometry.materials = std::move(materialInfos);
    geometry.textures = std::move(textureInfos);
    return geometry;
}
