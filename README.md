...
| **Virtual shadow maps** | An *absolute* clipmap page grid over the light plane — 128-texel pages, 32 per level edge, up to 12 levels — backed by a 1024-page physical pool, marked from the depth the Hi-Z pyramid already holds rather than from a prepass. A page's world rect never moves with the camera, which is what lets a warmed clipmap draw 0 pages a frame. Off by default |
| **Asset pipeline** | Offline BC7/KTX2 texture cook (Sponza 365.98 → 91.06 MiB, **4.02x**, decode wait to zero) and an offline mesh + LOD cook (glTF import 331.49 → 15.50 ms, **21x**), over a batched upload path with an optional dedicated transfer queue |
...
```mermaid
flowchart TD
    A[Scene update<br/>build draw items + upload object data] --> B[Shadows<br/>one caster cull for every cascade, cached cascades,<br/>punctual atlas, optional virtual shadow map pages]
    A --> D[Cluster build + light cull<br/>Forward+ froxel grid, on the async compute queue]
    B --> C[GPU frustum culling + LOD selection<br/>phase-1 Hi-Z occlusion → indirect commands]
    C --> P[Depth prepass<br/>opaque + alpha-tested]
    D --> P
    P --> E[Main HDR pass<br/>bindless + multi-draw indirect + clustered shading]
    E --> R[Phase-2 occlusion re-test<br/>mid-frame pyramid, rescued draws]
    R --> S[SSR, optional GTAO,<br/>transparent pass]
    S --> F[Depth pyramid<br/>for next-frame occlusion]
    S --> T[Optional TAA resolve<br/>temporal upsampling]
    T --> G[Bloom + histogram auto-exposure]
    G --> H[Composite + tonemap + sharpen]
...
- Ground-truth ambient occlusion (GTAO), off by default: a half-resolution horizon-search pass reuses the thin G-buffer normal and the depth buffer to integrate the cosine-weighted visibility arc over several slices, a joint-bilateral pass denoises and upsamples it against full-res depth, and the main pass applies the previous frame's term, reprojected along the motion vector, to the ambient light alone — direct light is not darkened. The whole-scene composite multiply it replaced stays as an A/B reference, off by default.
- Render scale, native by default: every screen-space target from main depth through the bloom chain is sized to an internal render extent, and the composite upscales to the swapchain while the ImGui overlay stays crisp. The frame is fragment-bound and its remaining cost is real shading work rather than waste, so pixel count is the lever that is left: at 2560x1440 on an RTX 3080 Ti, scale 0.5 takes the default scene from 6.73 to 2.10 ms (-69%) and `MainHDRPass` from 4.40 to 1.26 ms (-71%), clocks pinned, 63 samples a point, with the repeated control back within 0.81%. TAA jitter and LOD selection follow the render extent, since both are defined in rendered pixels. See [docs/render_scale.md](docs/render_scale.md).
...
- Virtual shadow maps, off by default: an **absolute** clipmap page grid over the light's XY plane, one grid per level with level *L*'s pages twice the world size of level *L-1*'s -- 128-texel pages, 32 per level edge, up to 12 levels -- backed by a 4096x4096 physical pool of 1024 pages. The virtual set is 12288 pages against that pool, so the page table is a real indirection with a residency policy behind it rather than a lookup that always hits. The grid being absolute is the whole reason caching works: a clipmap centred on the camera and snapped to texels hands every page a slightly different world rect every frame, so every page's contents change every frame and nothing is ever reusable. The camera only decides which finite window of that infinite grid is addressable, and a page's slot inside the window is its absolute coordinate *modulo* the axis rather than its offset from the window's corner -- window-relative indexing would renumber all 1024 slots the moment the window scrolled by one page, while wrapping renumbers only the row that actually changed identity. Marking reads the previous frame's depth from the Hi-Z pyramid, because it runs before this frame's depth prepass has written anything. Steady state on the default scene is 99 resident pages, all cached, **0 drawn per frame**. See [docs/virtual_shadow_maps.md](docs/virtual_shadow_maps.md).
- Offline texture cook to BC7 in KTX2 (`tools/cook_textures.py` over the `vecook` encoder): Sponza's textures go **365.98 -> 91.06 MiB (4.02x)** with 72 of 77 block compressed, device-local usage falls 256 MiB, and texture upload drops 46% while getting *simpler* -- a cooked texture ships its mip chain baked, so the runtime stops decoding on the CPU (decode wait 58.88 ms -> exactly 0.00 ms, which is categorical rather than a speedup) and stops generating mips with `vkCmdBlitImage`, leaving two barriers where there were many. That simplification is what made the transfer-queue path legal at all. The cook is never run from CMake -- a clean configure must not silently spend a minute encoding.
- Offline mesh + LOD cook (`tools/vemeshcook`): glTF import falls **331.49 -> 15.50 ms (21x)** and renderer init ~466 -> ~129 ms, because baking the simplified LOD chain removes work that parallelising it could only spread -- import was measured at 87% `meshopt_simplify` -- 823 of 942 main-thread profile samples, against 3 for JSON parsing. Staleness is checked rather than assumed, and a rejected cook says so in a log line instead of becoming an unexplained slow startup.
- Optional dedicated transfer queue for load-time uploads, with the ownership-transfer barriers that requires and a graphics-queue fallback. Enabled automatically where a TRANSFER-only family exists. It is a capability rather than a speedup and is documented as one: measured 21.99 -> 21.13 ms inside a 42.6% spread, which is no result, because the load flow still waits for the copies rather than overlapping them with anything.
- Pipelines are looked up by state through a `PipelineKey` rather than held in named members: graphics and compute alike, with identical requests collapsing to one object. Membership is decided by lifetime, not by pipeline type -- only what pipeline recreation rebuilds may live in the store, so eight subsystem-owned compute pipelines deliberately stay out -- and a store reset is generation-guarded, because every handed-out reference must be reissued and that invariant broke within one commit of being written.
- Back-face culling on the opaque and skinned main pipelines, on by default, with `Material::doubleSided` selecting a second un-culled pipeline and skybox, probe capture and the transparent pass staying un-culled for documented correctness reasons. It was off for most of this renderer's life and that was a measurement, not an oversight: on a tile-based GPU the hidden-surface removal already discards the back faces before the fragment shader, so there was nothing to save. **The conclusion did not survive the move to an immediate-mode GPU** -- `MainHDRPass` -37.4% and frame total -14.9% on an RTX 3080 Ti, clock-pinned, A/B/A/B x3 against 0.33% control drift.
...
- Async compute: ClusterBuild/LightCull run on a dedicated compute queue so they can overlap the CSM shadow passes — submitted before graphics recording even starts, synchronized by a per-frame semaphore waited at every stage that reads their output (the main pass's fragment shader, plus fog injection's compute dispatch when fog is on), with concurrent-sharing buffers across queue families and a graphics-queue fallback when no async queue exists.
...
| Async compute (cluster passes on their own queue) | [async_compute.md](docs/async_compute.md) |
| Punctual (spot/point) shadows | [punctual_shadows.md](docs/punctual_shadows.md) |
| Virtual shadow maps | [virtual_shadow_maps.md](docs/virtual_shadow_maps.md) |
...
| Transparency and alpha modes | [transparency.md](docs/transparency.md) |
| GPU culling + Hi-Z occlusion | [gpu_culling.md](docs/gpu_culling.md) |
| Mesh LOD, and the meshlet measurement | [mesh_lod.md](docs/mesh_lod.md) |
| GPU profiler | [profiling.md](docs/profiling.md) |
| Task-parallel frame preparation | [parallel_frame_prep.md](docs/parallel_frame_prep.md) |
| Asset/material system, texture + mesh cook | [asset_system.md](docs/asset_system.md), [asset_load_baseline.md](docs/asset_load_baseline.md) |
...
| Headless render CI and the configuration sweep | [headless_ci.md](docs/headless_ci.md) |
...
- `RenderGraph` infers conservative barriers and pass liveness, derives the execution order from the declared dependencies, and checks each frame that the passes recorded are the ones declared. It is still not a production scheduler: memory aliasing covers the bloom chain and the velocity/thin-G-buffer pair and is off by default (measured and lost: +1.2% of frame time for memory that is not scarce), physical allocation stays in `Renderer`, and async compute is handled outside the graph (the cluster passes manage their own barriers) rather than by a multi-queue scheduler.
- GPU shadow culling is on by default and draws cutout casters through the alpha-tested shadow pipeline, but it still consumes CPU-built draw items/batches and is not occlusion-driven or BVH-backed.
- CSM offers two fits. The default fits each cascade to its slice's light-space AABB with texel snapping, which is rotation-dependent: turning the camera changes the ortho extent, the world-units-per-texel derived from it, and so the snapping grid itself. An optional bounding-sphere fit makes the extent depend only on near/far/fov/aspect, which removes that shimmer at the cost of sharpness, since a sphere around the slice is wider than the slice. It is off by default and **measured not to help the cascade cache**: the slice centre orbits the camera, so a rotation moves it by roughly the centre distance times the angle, and the matrix survives only 0.0025-0.041 degrees of yaw where a camera turning at 10 degrees per second covers 0.17 degrees per frame. Per-cascade resolution control is still not implemented.
- Virtual shadow maps are off by default and gated behind three nested toggles (mark, render, sample). They are bindless-path only -- `simple.frag`, the fallback for devices without descriptor indexing, still uses the cascades. Effective resolution is capped by the pages-per-level axis rather than by `level0Extent`: at 32 that is one texel per pixel at 4K, and above 4K the clipmap saturates again. Page casters draw authored geometry with no LOD, the request set lags by the frames in flight, and page counts have only been measured on the default and geometry-stress scenes, both with a static camera.
...
- The TAA resolve doubles as the upsampler, reconstructing at presentation resolution from the jittered low-resolution samples, but it is not FSR2 or DLSS: no locks, no reactive masks, and no disocclusion detection (the resolve reads no previous-frame depth). With TAA off, render scale upscales bilinearly plus a contrast-adaptive sharpen, which can only re-emphasise what survived. Sub-rect rendering makes a scale change free but costs memory: a 0.5-scale frame owns four times the texels it writes.
...
- BVH/spatial partitioning and mesh/task shader experiments. Discrete mesh LOD has since shipped, and a meshlet builder with it — meshlet culling itself was measured and rejected here, because LOD already removes the geometry it would have found ([docs/mesh_lod.md](docs/mesh_lod.md)).
- Improved HDR environment prefiltering, color-management policy, local exposure, and HDR swapchain output.
- Expanded scene editing (object creation/deletion, hierarchy editing, per-scene settings) and asset tooling (texture import/reload, asset browser, material graph, render graph node view). Click-to-pick and a transform gizmo have since shipped.
...
> C++20 Vulkan 1.3 real-time renderer with GPU-driven visibility (bindless materials, multi-draw indirect, compute frustum culling that also picks each draw's LOD, two-phase Hi-Z occlusion, a depth prepass), clustered (Forward+) lighting on an async compute queue, three cached shadow paths (cascades, a punctual atlas, virtual shadow maps), SSR, GTAO, probe GI and volumetric fog, a TAA resolve that doubles as a temporal upsampler behind render scale and dynamic resolution, GPU skeletal animation, an offline texture and mesh cook, and a render graph whose inferred barriers are checked by synchronization validation in CI.

Resume bullets:

- Built a C++20 Vulkan 1.3 renderer on Dynamic Rendering, Synchronization2, VMA and Volk, organised by a render graph that infers barriers, derives pass order from declarations, and checks every frame that what was recorded is what was declared.
- Engineered a GPU-driven pipeline: bindless textures, multi-draw indirect, compute frustum culling with per-draw LOD selection, two-phase Hi-Z occlusion that suspends itself while it culls nothing, and a depth prepass measured at −13.0% of frame time on Sponza (RTX 3080 Ti Laptop, 1280x720, p10, A/B/A/B).
- Implemented clustered (Forward+) lighting — a compute-built 32×18×24 froxel grid on an async compute queue — scaling to hundreds of dynamic point/spot lights, with spot and point shadows in a cached atlas whose hardware-compare filter took 21.9% off the main pass on the same Sponza setup.
