...
| **Shadows** | PCF cascaded shadow maps with per-cascade GPU shadow-caster culling, an indirect shadow path, an alpha-tested pipeline so cutout casters throw perforated shadows, and a 4096px spot-light shadow atlas so punctual lights cast too |
...
- Punctual (spot) shadows via a 4096x4096 shared depth atlas of 64 tiles. One rendering scope covers the whole atlas and each slot is isolated by viewport/scissor, so the clear runs once and empty tiles stay cleared. The slot index rides in `GpuLight`'s unused padding so the light record stays 64 bytes, and the negative sentinel that means "does not cast" is also what guards the slot-buffer dereference in the shader. Registered in the render graph, so the layout transitions are inferred rather than hand-written. Point lights need six cube faces and do not cast yet. See [docs/punctual_shadows.md](docs/punctual_shadows.md).
