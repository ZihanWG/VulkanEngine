...
12. When `renderer.enableDepthPrepass` is on (off by default), replay the opaque and masked buckets depth-only into the main depth image as `DepthPrepass`, so the pass below loads that depth instead of clearing it and its `LESS_OR_EQUAL` test lets early-Z reject fragments it would otherwise shade and overwrite. Masked geometry runs the same cutout test the shadow path uses; blended geometry is never prepassed. See `design_decisions.md`.
