...
- [GPU Culling](gpu_culling.md): GPU frustum culling, two-phase Hi-Z occlusion
  (on by default, with a same-frame re-test), occlusion test scene, counters, and
  fallbacks.
- [Async Compute](async_compute.md): The async compute queue selection,
  submitting ClusterBuild/LightCull so they can overlap the shadow passes, the
  semaphore's wait scope and what it orders, and concurrent-sharing buffers.
...
- [Temporal AA](taa.md): Optional motion-vector TAA whose resolve doubles as
  the temporal upsampler, Halton jitter, Catmull-Rom history, invalidation, debug
...
- [Asset Load Baseline](asset_load_baseline.md): Measured startup asset load
  cost, the `--asset-load-stats` instrumentation, and the before-and-after
  numbers for the texture cook, the mesh cook and the upload work.
...
C++20 Vulkan 1.3 real-time renderer featuring GPU-driven visibility (compute
frustum culling with per-draw LOD, two-phase Hi-Z occlusion, a depth prepass),
clustered (Forward+) lighting on an async compute queue, cached cascaded,
punctual and virtual shadow maps, SSR, GTAO, probe GI, volumetric fog, a TAA
resolve that doubles as a temporal upsampler, GPU skeletal animation, an offline
texture and mesh cook, a render graph with checked barrier inference, and GPU
timestamp profiling. The root README carries the full summary.
